...
- [blocked] Release branch `codex/release-v1-6-24` is pushed at `c45ecb0`, but this workstation's GitHub CLI authentication returned HTTP 401 while creating the PR. No PR, `main` merge, tag, GitHub Release, Windows Setup artifact, or public download has been created.
- [next] After restoring `gh auth login` access, create/merge the release PR to `main`, then verify the public v1.6.24 DMG, Setup.exe, and SHA256SUMS.txt. Windows Setup compilation remains a required GitHub Actions Windows gate.
