...
- v1.6.43 retries transient signed Nexo skin image connection failures before reporting one-click apply failure; URL, redirect, media, size, hash, and renderer checks remain unchanged.
...
- v1.6.29 随 macOS 全新安装等待修复同步发布版本号；Windows 仍保持等待 Codex 自然退出后继续安装，不强制关闭或重启 Codex。
- v1.6.28 固定平台 Ed25519 目录公钥并支持带完整界面视觉配置的签名目录 v2；新增审核皮肤可由平台动态下发，无需重构助手。同步修复 Windows 主题 ID 与网页设置 ID 不一致，并让双端只从已验证缓存扩展字体、颜色和侧边栏设置白名单。
- v1.6.27 同步樱花信使固定资源的 SHA-256 回归基准，并在目录测试中校验 Swift 固定哈希与目录资源始终一致，修复 macOS 发布单测失败。
- v1.6.26 新增 59 套已授权的超分皮肤，固定目录扩展到 124 套；每套均包含完整 Theme V2 配置，覆盖背景、侧栏、内容层、字体、输入框与窗口区域。
- v1.6.25 修复预览文字颜色在注入末尾被主题默认 RGB 变量覆盖的问题；邮差浣熊侧栏移除点纹并显示原背景图，云鹿栖境改用青灰面板、深墨文字和高对比输入框；双平台运行时资源保持一致。
- v1.6.24 修复本机平台调试页被外观桥接错误拒绝的问题：仅允许 `localhost:5173` 与 `127.0.0.1:5173` 两个 Vite 环回来源同步设置；文字颜色、字体及透明度现在会写入当前皮肤并热更新。邮差浣熊同步为原画的深蓝、邮包红与暖纸色调，移除导致画面漂白的浅纸张面板。
- v1.6.23 与 macOS 更新器收尾修复同步版本号；Windows 安装与更新行为不变。
- v1.6.22 与 macOS 更新修复版同步版本号；Windows 更新仍保持校验后暂存并等待 Codex 自然关闭。
- v1.6.21 同步平台当前 65 套固定皮肤及真实资源文件名，修复删除目录项后暖景41等皮肤仍请求旧编号图片而下载失败的问题。
- v1.6.20 补齐发布形态回归夹具中的外观桥接资源，确保 PowerShell 5.1/7 在正式发布提交上验证完整运行时。
- v1.6.19 将平台皮肤预览中的背景、侧栏、内容层、字体、字号和文字对比度安全同步到本机助手，并按皮肤独立保存后热更新当前窗口。
- v1.6.18 修复所有窗口尺寸下的侧栏：基础侧栏不再覆盖固定黑色渐变，透明时显示同一张皮肤画面。
- v1.6.17 修复任务窗口的侧栏透明度：沉浸式侧栏不再固定为深色遮罩，始终读取本机外观设置。
- v1.6.16 新增本机外观设置，可调整背景、侧边栏、内容层、字体、字号和文字对比度；设置重新应用后立即生效。
- v1.6.15 不再在 Codex 首页注入皮肤名称；应用后只保留原生页面内容。
- v1.6.14 隐藏皮肤主页中的“选择项目 · 项目名称”标签，保留原生主页与皮肤标题。
- v1.6.13 将全部 87 套皮肤的真实主题名称统一为五个字以内；移除原始素材标题中的壁纸、背景、动态、视频等文案，霓虹城市主题显示为“霓虹夜城”。
- v1.6.12 所有皮肤首页只显示最多五个字的短标题并移除壁纸类营销词；“4K 深蓝礼服”固定显示“深蓝礼服”，同时修复窄窗口重新露出原始长标题的问题。
- v1.6.11 恢复运行时生成资产的固定 LF 检出规则，修复 PowerShell 5.1/7 下的字节一致性构建失败。
- v1.6.10 将所有皮肤首页标题压缩为短名称并移除重复副标题；仅“4K 深蓝礼服”皮肤关闭散点装饰，其他皮肤视觉装饰保持不变。
- v1.6.9 同步 87 套已完成平台授权审核的固定皮肤；平台 Windows 首次一键应用会引导安装官方 Setup.exe 后继续应用。
- v1.6.8 修复 macOS 最新版被旧引擎版本误判的问题，更新提示增加 Release 更新说明；六类主题同步增加首页、快捷卡片、顶部框架与新窗口特效。
- v1.6.7 与 macOS 最终修复版同步发布版本号；Windows 固定目录与安全协议保持不变。
- v1.6.6 与 macOS 正式修复版同步发布不可变版本标签；Windows 固定目录与安全协议保持不变。
- v1.6.5 与 macOS 修复版同步发布版本号；Windows 固定目录与安全协议保持不变。
- v1.6.4 修复自动更新后 Codex 尚未关闭时过早安装组件的英文报错：助手现在会静默等待 Codex 正常关闭，再自动完成组件更新；发布流水线不再重复执行完整 Windows 双 Shell 矩阵，并缓存固定 Node 运行时以缩短构建时间。
- v1.6.3 将助手完整收口为 Nexo Codex Skin：macOS/Windows 统一 Nexo 同心环图标、安装器与更新提示品牌，并把低频操作折叠进“高级工具”；保留原有安装身份和一键协议，旧版本可安全自动升级。
- v1.6.2 完成 NexoToken 品牌 README、修正移除外部入口后的 Windows 发布断言，并将最终安装包绑定到包含全部修复的新版本标签。
- v1.6.1 移除托盘中的外部主题库、在线 Studio 与第三方网站入口，付费皮肤只通过 Nexo 平台授权的一键链接分发；同时固定 Theme V2 源文件在 Windows 检出时使用 LF，避免生成资产被误报为过期。

### 修复

- Nexo 固定皮肤目录扩展到 18 套，并加入安全自动更新：托盘启动后每日检查一次；旧客户端遇到未来的固定皮肤编号时直接进入更新流程。用户确认后下载固定 GitHub Release 的 Setup 与校验文件并核对 SHA-256；Codex 正在运行时只暂存更新，等用户自然关闭后才自动安装并重新启动托盘，不强制关闭或重启 Codex。
...
- 同步 v1.5.14 版本号，并刷新迁移到自有公开仓库后的发布校验。
- 同步 v1.5.13 版本号，发布 18 套 Nexo 主题与安全自动更新。
- 同步 v1.5.12 版本号，发布 Nexo 原生一键换肤协议。
