...
const V1_SKIN_KEYS = ['appearance', 'backgroundPath', 'backgroundSha256', 'category', 'id', 'nameEn', 'nameZh', 'previewPath', 'previewSha256', 'tags'];
const V2_SKIN_KEYS = [...V1_SKIN_KEYS, 'taskMode', 'visual'];
const VISUAL_KEYS = ['accentRGB', 'composerStyle', 'cornerStyle', 'focusX', 'focusY', 'glowStrength', 'layoutVariant', 'motionPreset', 'panelRGB', 'secondaryRGB', 'sidebarStyle', 'signature', 'surfaceStyle', 'textureStyle'];
const RGB = /^(?:0|[1-9][0-9]?|1[0-9]{2}|2[0-4][0-9]|25[0-5]) (?:0|[1-9][0-9]?|1[0-9]{2}|2[0-4][0-9]|25[0-5]) (?:0|[1-9][0-9]?|1[0-9]{2}|2[0-4][0-9]|25[0-5])$/;
const VISUAL_ENUMS = Object.freeze({
  layoutVariant: new Set(['collage', 'console', 'editorial', 'pixel-console', 'pixel-desktop', 'pixel-platform', 'poster-right', 'stage']),
  surfaceStyle: new Set(['glass', 'ink', 'metal', 'paper', 'pixel']),
  cornerStyle: new Set(['cut', 'pixel', 'round', 'stamp', 'tape', 'ticket']),
  motionPreset: new Set(['cursor', 'doodle', 'ink', 'mail', 'mist', 'none', 'orbit', 'petals', 'pixel-rain', 'rain', 'scan', 'sonar', 'sparks', 'spotlight']),
  sidebarStyle: new Set(['aurora', 'blueprint', 'file-tree', 'forge', 'garden', 'harbor', 'maritime', 'navigation', 'neon', 'notebook', 'postal', 'scroll', 'setlist', 'station', 'submarine', 'terminal']),
  composerStyle: new Set(['console', 'dialog', 'label', 'letter', 'mixer', 'pixel-console', 'sonar', 'terminal', 'workbench']),
  textureStyle: new Set(['crayon', 'dither', 'droplets', 'grain', 'grid', 'paper', 'scanline', 'vinyl', 'wash']),
});
...
function validateVisualProfile(visual) {
  exactObject(visual, VISUAL_KEYS, 'visual');
  for (const field of ['accentRGB', 'secondaryRGB', 'panelRGB']) {
    if (typeof visual[field] !== 'string' || !RGB.test(visual[field])) throw new TypeError(`${field} is invalid`);
  }
  const normalized = {
    ...visual,
    signature: boundedText(visual.signature, 'signature', 64),
    focusX: boundedNumber(visual.focusX, 'focusX', 0, 1),
    focusY: boundedNumber(visual.focusY, 'focusY', 0, 1),
    glowStrength: boundedNumber(visual.glowStrength, 'glowStrength', 0, 1),
  };
  for (const [field, allowed] of Object.entries(VISUAL_ENUMS)) {
    if (typeof visual[field] !== 'string' || !allowed.has(visual[field])) throw new TypeError(`${field} is invalid`);
  }
...
function validatePayload(payload, now, { allowExpired = false } = {}) {
  ...
  if (![1, 2].includes(payload.schemaVersion) || !Number.isSafeInteger(payload.catalogVersion) || payload.catalogVersion < 1) throw new TypeError('catalog version is invalid');
  ...
  const skins = payload.skins.map((skin) => {
    exactObject(skin, payload.schemaVersion === 2 ? V2_SKIN_KEYS : V1_SKIN_KEYS, 'skin');
    ...
    if (payload.schemaVersion === 2 && skin.taskMode !== 'full') throw new TypeError('taskMode is invalid');
    const visual = payload.schemaVersion === 2 ? validateVisualProfile(skin.visual) : null;
    return Object.freeze({
      ...
      source: 'remote',
      taskMode: payload.schemaVersion === 2 ? skin.taskMode : 'full',
      visual,
    });
  });
