...
function Wait-DreamSkinCodexClosedForSetup {
  while ($true) {
    ...
    if ($Silent) { throw 'Close Codex before installing or updating Nexo Codex Skin.' }
    Add-Type -AssemblyName System.Windows.Forms
    $choice = [System.Windows.Forms.MessageBox]::Show(
      'Codex is currently running. Close it, then click Retry to continue setup.',
      'Nexo Codex Skin Setup',
...
try {
  ...
  if ($Uninstall) {
    ...
    if ($restoreRequired -and -not (Test-Path -LiteralPath $engine.Restore -PathType Leaf)) {
      throw 'The installed restore engine is missing. Reinstall Nexo Codex Skin, then uninstall again so Codex can be restored safely.'
    }
    if ($restoreRequired) {
      $restoreParameters = @{
        Uninstall = $true
  ...
  if ($installedVersion -cmatch '^(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)$' -and
    ...
    throw "A newer Nexo Codex Skin v$installedVersion is already installed. Download that version or newer instead of downgrading to v$payloadVersion."
  }
  ...
  $requiredEngineFiles = @(
    ...
    'scripts\signed-nexo-catalog.ps1',
