...
$script:DreamSkinNexoCatalogPath = Join-Path (Split-Path $PSScriptRoot -Parent) 'assets/nexo-skin-catalog.json'
$script:DreamSkinNexoCatalogDocument = ConvertFrom-Json (Read-DreamSkinUtf8File -Path $script:DreamSkinNexoCatalogPath)
if ($script:DreamSkinNexoCatalogDocument.schemaVersion -ne 2 -or
    $script:DreamSkinNexoCatalogDocument.assetOrigin -notmatch '\Ahttps://') {
  throw 'The generated Nexo skin catalog is invalid.'
}
$script:DreamSkinNexoAssetOrigin = [string]$script:DreamSkinNexoCatalogDocument.assetOrigin
$script:DreamSkinNexoCatalogById = @{}
foreach ($item in @($script:DreamSkinNexoCatalogDocument.items)) {
  $script:DreamSkinNexoCatalogById[[string]$item.id] = $item
}

function ConvertFrom-DreamSkinHexColor {
  param([Parameter(Mandatory = $true)][string]$Hex)
  if ($Hex -notmatch '\A#[0-9a-fA-F]{6}\z') { throw 'Generated theme color is invalid.' }
  return '{0} {1} {2}' -f [Convert]::ToInt32($Hex.Substring(1, 2), 16),
...
function Resolve-DreamSkinNexoApplyUri {
  ...
  return [pscustomobject]@{
    ...
    SecondaryRGB = ConvertFrom-DreamSkinHexColor -Hex ([string]$record.theme.colors.secondary)
    PanelRGB = ConvertFrom-DreamSkinHexColor -Hex ([string]$record.theme.colors.panel)
    GlowStrength = $(if ($record.theme.family -eq 'cinematic-cyber') { 0.64 } else { 0.50 })
    Signature = ([string]$record.id).Replace('-', ' ').ToUpperInvariant()
    FocusX = [double]$record.theme.art.focusX
    FocusY = [double]$record.theme.art.focusY
    LayoutVariant = [string]$record.theme.visual.layout
    SurfaceStyle = [string]$record.theme.visual.surface
    CornerStyle = [string]$record.theme.visual.corners
    MotionPreset = [string]$record.theme.visual.motion
    SidebarStyle = [string]$record.theme.visual.sidebar
    ComposerStyle = [string]$record.theme.visual.composer
    TextureStyle = [string]$record.theme.visual.texture
  }
}
