...
- 新增 Nexo 固定皮肤目录协议 `dreamskin://apply?skin=...` 与恢复协议；只接受 12 个内置编号和固定 HTTPS 图片来源。修复新版 Codex 经 LaunchServices 启动时静默丢失 CDP 参数的问题，改为直接启动已验签官方可执行文件，并仅在可见渲染验证通过后报告成功。菜单栏 App 为 Universal 版本，同时支持 Apple 芯片与 Intel Mac。
