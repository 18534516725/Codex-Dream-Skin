...
- v1.6.43 synchronizes the release version with the Windows transient Nexo skin image retry repair; macOS runtime behavior is unchanged.
...
- v1.6.31 首次一键应用会自动将新助手连接到浏览器当前登录的 NexoToken 账号：一次性连接码只在 URL 片段中传递并在确认后清除，不要求用户选择账号、复制连接码或在助手中输入登录信息；连接成功后自动继续原来的皮肤应用。
- v1.6.30 将 Nexo 设备签名身份从 macOS 钥匙串迁至助手私有 Application Support 文件；新下载客户端不再请求钥匙串密码，仍使用 Ed25519 签名完成账号连接和皮肤权限校验。
- v1.6.29 修复全新安装时 Codex 正在运行会直接显示英文安装错误的问题：助手会保留待应用皮肤，提示用户保存并正常退出 Codex，检测退出后自动完成组件安装并继续一键应用；不会强制关闭 Codex。
...
- v1.6.25 修复预览文字颜色在注入末尾被主题默认 RGB 变量覆盖的问题；邮差浣熊侧栏移除点纹并显示原背景图，云鹿栖境改用青灰面板、深墨文字和高对比输入框；换肤期间的平台连接等待延长，已安装用户不再被误弹首次安装提示。
...
- v1.6.23 修复 macOS 将 `/tmp` 规范化为 `/private/tmp` 后更新器无法识别并卸载 DMG 的问题，安装成功后不再残留临时挂载。
- v1.6.22 自动更新下载遇到 GitHub 瞬时失败时有界重试；更新失败会重新打开仍有效的旧助手，不再让菜单栏助手消失。
- v1.6.21 同步平台当前 65 套固定皮肤及真实资源文件名，修复删除目录项后暖景41等皮肤仍请求旧编号图片而下载失败的问题。
- v1.6.20 补齐 Windows 发布形态回归夹具中的外观桥接资源，确保 PowerShell 5.1/7 在正式发布提交上验证完整运行时。
...
- v1.6.16 修复助手“打开 ChatGPT”绕过换肤启动器的问题；现在从助手启动会保留皮肤模式。新增本机外观设置，可调整背景、侧栏、内容层、字体、字号和文字对比度；应用后的主页不显示皮肤名称。
...
- v1.6.11 恢复 Windows 运行时生成资产的固定 LF 检出规则，修复字节一致性检查在 PowerShell 5.1/7 上失败的问题。
- v1.6.10 将所有皮肤首页标题压缩为短名称并移除重复副标题；仅“4K 深蓝礼服”皮肤关闭散点装饰，同时修复 macOS 固定皮肤 ID 在生成主题时丢失的问题。
- v1.6.9 将已完成平台授权审核的 69 套素材加入固定目录（共 87 套），并让 Windows 首次一键应用明确引导到官方安装包后继续应用。
- v1.6.8 修复最新版助手被旧引擎 VERSION 误判为需要更新的问题，更新确认增加安全截断的 Release 说明；六类主题同步增加首页、快捷卡片、顶部框架与新窗口特效。
- v1.6.7 将 18 套审核目录以只读资源和编译内置备份双重打包，彻底消除正式应用与 XCTest 资源路径差异。
- v1.6.6 补齐 SwiftPM/XCTest 的有界目录查找，并将正式修复绑定到不可变的新版本标签。
- v1.6.5 修复 macOS 菜单栏助手处理一键换肤链接时因安装包缺少 SwiftPM 资源 bundle 而闪退的问题；固定目录改从签名应用资源安全读取，资源缺失时拒绝应用而不崩溃。
...
- v1.6.1 移除菜单栏中的外部主题库、在线 Studio 与第三方网站入口，付费皮肤只通过 Nexo 平台授权的一键链接分发；同时固定 Theme V2 源文件在 Windows 检出时使用 LF，避免生成资产被误报为过期。

### 修复

- Nexo 固定皮肤目录扩展到 18 套，并加入安全自动更新：助手启动后每日检查一次；旧客户端遇到未来的固定皮肤编号时直接进入更新流程。用户确认后只下载固定 GitHub Release 的 DMG 与校验文件，核对 SHA-256、App 身份、版本和架构后事务替换并重启助手，不关闭或重启 Codex。
...
- 同步 v1.5.14 版本号，并刷新迁移到自有公开仓库后的固定主题元数据校验值。
- 同步 v1.5.13 版本号，发布 18 套 Nexo 主题与安全自动更新。
- 同步 v1.5.12 版本号，发布 Nexo 原生一键换肤与新版 Codex 启动修复。
