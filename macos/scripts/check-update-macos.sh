...
fail() {
  printf 'Nexo Codex Skin update check: %s\n' "$*" >&2
  exit 1
}
...
on run argv
  display dialog "发现新版本 " & (item 1 of argv) & return & return & \
    ...
    default button "前往下载" with title "Nexo Codex Skin"
end run
APPLESCRIPT
    then
...
on run argv
  display alert "Nexo Codex Skin" message "当前已是最新版本 " & (item 1 of argv) buttons {"好"}
end run
APPLESCRIPT
  fi
fi
