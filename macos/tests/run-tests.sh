...
for required_runtime in apply-community-theme-macos.sh snapshot-active-theme-macos.sh \
  theme-content-fingerprint.mjs theme-switch-lock-macos.sh install-update-macos.sh; do
...
"$NODE" "$ROOT/tests/native-immersive-css.test.mjs"
...
/usr/bin/grep -F -q 'assets/portal-hero.png' \
  "$STANDALONE_ROOT/docs/reference-background-prompt-guide.md"
/usr/bin/grep -F -q 'https://github.com/18534516725/Codex-Dream-Skin/blob/main/windows/assets/theme.json' \
  "$STANDALONE_ROOT/docs/reference-background-prompt-guide.md"
...
/usr/bin/grep -F -q \
  'https://github.com/18534516725/Codex-Dream-Skin/blob/main/windows/assets/theme.json' \
  "$REPACK_GUIDE"
...
/usr/bin/env -u HOME EXPECTED_VERSION="$(/usr/bin/tr -d '[:space:]' < "$ROOT/VERSION")" \
  /bin/bash -c '. "$1/scripts/common-macos.sh"; [ -n "$HOME" ] && [ "$SKIN_VERSION" = "$EXPECTED_VERSION" ]' _ "$ROOT"
